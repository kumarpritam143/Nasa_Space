...
// import { mockNeoAsteroids } from '../mock';
const NASA_API_KEY = process.env.REACT_APP_NASA_API_KEY;
const NASA_API_URL = process.env.REACT_APP_NASA_API_URL;
// LIVE fetch function inside the component (or import from mock.js if you want)
const fetchNeoAsteroids = async () => {
  try {
    const today = new Date().toISOString().split('T')[0];
    const apiKey = '1g8EgTht6M3OZXy7wU3coOJAZIV8K8WEcHe1ZGhc';
...
const InputPanel = ({ onSimulate, isSimulating }) => {
  ...
  const [customParams, setCustomParams] = useState({ diameter: 100, velocity: 15, angle: 45 });
  const [mockNeoAsteroids, setMockNeoAsteroids] = useState([]); // state for live data

  // fetch live data on component mount
  useEffect(() => {
    const loadAsteroids = async () => {
      const data = await fetchNeoAsteroids();
      setMockNeoAsteroids(data);
    };
    loadAsteroids();
  }, []);
      ...
      <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm">
        ...
        <CardContent>
          <Tabs value={inputMode} onValueChange={setInputMode} className="space-y-6">
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <div><strong>NASA JPL URL:</strong> <a href={asteroid.nasa_jpl_url} target="_blank" className="text-blue-600 underline ml-1">View</a></div>
            ...
            {/* custom input remains same */}
            <TabsContent value="custom" className="space-y-6">
              {/* ... sliders for custom asteroid ... */}
            </TabsContent>
          </Tabs>
